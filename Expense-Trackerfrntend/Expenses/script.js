...
function onsubmit(e){
    ...
    axios.post('http://16.171.230.180:3000/expenses/addexpense',myoj,{headers:{"Authorization":token}})
    .then(res=>{
...
function deletefn(userid){
    console.log(userid)
    const token=localStorage.getItem('token');
    axios.delete(`http://16.171.230.180:3000/expenses/delete/${userid}`,{headers:{"Authorization":token}})
            .then((response)=>{
...
async function getexpenses(page){
    ...
    axios.get(`http://16.171.230.180:3000/expenses/getexpense?page=${page}&lim=${lim}`,{headers:{"Authorization":token}})
    .then((res)=>{
...
razorpay.onclick=async(e)=>{
    ...
    e.preventDefault();
    const token=localStorage.getItem('token');
    const response= await axios.get('http://16.171.230.180:3000/purchase/premiummembership',{headers:{"Authorization":token}})
    console.log(response);
    var options=
    {
        ...
        "handler":async function(response){
            await axios.post('http://16.171.230.180:3000/purchase/updatetransactionstatus',{
...
async function  showleaderboad (){
    ...
    const token=localStorage.getItem('token');
   const userlist= await axios.get('http://16.171.230.180:3000/premium/showleaderboard',{headers:{"Authorization":token}});
...
async function  downloadexpense(){
    const token=localStorage.getItem('token');
   const data = await axios.get('http://16.171.230.180:3000/expenses/download',{headers:{"Authorization":token}});
   if(data.status==200){
...
async function getdownloads(){
    try{
    const token=localStorage.getItem('token');
    const downloads=await axios.get('http://16.171.230.180:3000/expenses/get-downloads',{headers:{"Authorization":token}})
