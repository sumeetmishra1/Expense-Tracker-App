...
function onsubmit(e){
    ...
    axios.post('/expenses/addexpense',myoj,{headers:{"Authorization":token}})
    .then(res=>{
...
function deletefn(userid){
    console.log(userid)
    const token=localStorage.getItem('token');
    axios.delete(`/expenses/delete/${userid}`,{headers:{"Authorization":token}})
            .then((response)=>{
...
async function getexpenses(page){
    ...
    axios.get(`/expenses/getexpense?page=${page}&lim=${lim}`,{headers:{"Authorization":token}})
    .then((res)=>{
...
async function  showleaderboad (){
    ...
    const token=localStorage.getItem('token');
   const userlist= await axios.get('/premium/showleaderboard',{headers:{"Authorization":token}});
...
async function  downloadexpense(){
    const token=localStorage.getItem('token');
   const data = await axios.get('/expenses/download',{headers:{"Authorization":token}});
   if(data.status==200){
...
async function getdownloads(){
    try{
    const token=localStorage.getItem('token');
    const downloads=await axios.get('/expenses/get-downloads',{headers:{"Authorization":token}})
